            ...
            <div className="flex flex-col gap-5">
                {data.map((event) => (
                    <Link key={event.eventSlug} href={`/events/${event.eventSlug}`}>
                        <img className="cursor-pointer" style={{margin: theme.spacing.sm, aspectRatio: "1:1", width: "100%", maxWidth: 350, borderRadius: theme.borderRadius.xs, overflow: "hidden"}} alt={event.name+" poster image"} src={event.poster.url}/>
                    </Link>
                ))}
            </div>
        </Container>
